...
local function StartMainScript()
    ...
    local function CreateESP(v)
        local Lines = {}
        for i = 1, 12 do Lines[i] = Drawing.new("Line") Lines[i].Color = Color3.fromRGB(255, 0, 0) Lines[i].Thickness = 1.5 Lines[i].Visible = false end
        local Tracer = Drawing.new("Line") Tracer.Color = Color3.fromRGB(255, 0, 0) Tracer.Visible = false
    ...
    Title.TextSize = 20
    Instance.new("UIStroke", Title).Color = Color3.fromRGB(0, 255, 0)
    ...
    local function AddToggle(name, varName)
        ...
        local function Upd()
            ...
            b.TextColor3 = _G[varName] and Color3.fromRGB(255, 255, 255) or Color3.fromRGB(0, 255, 0)
            b.BackgroundColor3 = _G[varName] and Color3.fromRGB(0, 150, 0) or Color3.fromRGB(20, 20, 20)
    ...
    local function ShowAim() Content:ClearAllChildren() Instance.new("UIListLayout", Content).Padding = UDim.new(0, 5) AddToggle("Aimlock Head", "Aimlock") AddToggle("FlyKill (Next)", "FlyKill") AddToggle("MS20 Pull", "MS20") end
    ...
    local function ShowConfig() Content:ClearAllChildren() Instance.new("UIListLayout", Content).Padding = UDim.new(0, 5) AddToggle("Speed Run (5x)", "SpeedRunning") AddToggle("Speed Vehicle", "SpeedCar") AddToggle("Auto Wins", "AutoWins") end
...
LoginBtn.MouseButton1Click:Connect(function()
    if KeyInput.Text == CurrentKey then SaveTime() StartMainScript() else KeyInput.PlaceholderText = "WRONG!" KeyInput.Text = "" end
end)
