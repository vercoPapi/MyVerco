...
local function StartMainScript()
    ...
    local function CreateESP(v)
        ...
        RunService.RenderStepped:Connect(function()
            if _G.EspEnabled and IsEnemy(v) and v.Character and v.Character:FindFirstChild("HumanoidRootPart") and v.Character.Humanoid.Health > 0 then
                ...
                if OnScreen then
                    local size = Vector3.new(4, 5, 2); local cf = hrp.CFrame
                    local vts = {
    ...
    RunService.RenderStepped:Connect(function()
        ...
        if Target and Target.Character then
            -- PERBAIKAN FLYKILL: Diam di atas musuh 20 Meter
            if _G.FlyKill then
    ...
    Frame.BackgroundColor3 = Color3.fromRGB(5, 5, 5); Frame.ClipsDescendants = true
    Instance.new("UIStroke", Frame).Color = Color3.fromRGB(0, 255, 0)
    Frame.Active = true; Frame.Draggable = true

    for i = 1, 15 do
        local p = Instance.new("Frame", Frame); p.Size = UDim2.new(0, 2, 0, 2); p.BackgroundColor3 = Color3.fromRGB(0, 255, 0)
        local function anim()
    ...
    Title.Size = UDim2.new(1, 0, 0, 40); Title.BackgroundColor3 = Color3.fromRGB(0, 40, 0); Title.Text = "VERCO X NUGI"; Title.TextColor3 = Color3.fromRGB(0, 255, 0); Title.Font = Enum.Font.GothamBold; Title.TextSize = 22
    ...
    local function ShowAim() Content:ClearAllChildren(); Instance.new("UIListLayout", Content).Padding = UDim.new(0, 5); AddToggle("FlyKill Musuh (20m)", "FlyKill"); AddToggle("Aimlock Head", "Aimlock"); AddToggle("MS20 Pull", "MS20") end
    ...
    local function ShowConfig() Content:ClearAllChildren(); Instance.new("UIListLayout", Content).Padding = UDim.new(0, 5); AddToggle("Speed Running", "SpeedRunning"); AddToggle("Speed Vehicle", "SpeedCar"); AddToggle("Jump Multi", "JumpMulti"); AddToggle("Wallhack", "Wallhack"); AddToggle("Auto Wins Global", "AutoWins") end
    local function ShowSouthBronx() Content:ClearAllChildren(); Instance.new("UIListLayout", Content).Padding = UDim.new(0, 5); AddToggle("Auto Farm SouthBronx", "AutoFarmSB") end
    ...
    CreateTab("AIM", ShowAim); CreateTab("VISUAL", ShowVisual); CreateTab("CONFIG", ShowConfig); CreateTab("S.BRONX", ShowSouthBronx); CreateTab("RINTANGAN", ShowRintangan)
    ShowAim()

    local CTBtn = Instance.new("TextButton", MainGui); CTBtn.Size = UDim2.new(0, 50, 0, 50); CTBtn.Position = UDim2.new(0, 10, 0.5, 0); CTBtn.Text = "CT"; CTBtn.BackgroundColor3 = Color3.fromRGB(0, 80, 0); CTBtn.TextColor3 = Color3.fromRGB(0, 255, 0)
    CTBtn.MouseButton1Click:Connect(function() Frame.Visible = not Frame.Visible end)
end

-- ================= EXECUTION =================
LoginBtn.MouseButton1Click:Connect(function()
    if KeyInput.Text == CurrentKey then SaveTime(); StartMainScript() else KeyInput.Text = "SAL
