...
local function StartMainScript()
    ...
    RunService.RenderStepped:Connect(function()
        ...
        for _, v in pairs(game.Players:GetPlayers()) do
            if v ~= Player and v.Character and v.Character:FindFirstChild("HumanoidRootPart") and v.Character.Humanoid.Health > 0 then
                ...
                -- MS20 Meter (Narik musuh yang mendekat)
                if _G.MS20 and Mag < 25 then
        ...
        if Target and Target.Character then
            -- FlyKill Smart Low (Di atas kepala musuh jarak dekat)
            if _G.FlyKill then
                Player.Character.HumanoidRootPart.CFrame = Target.Character.HumanoidRootPart.CFrame * CFrame.new(0, 8, 0)
                Player.Character.HumanoidRootPart.Velocity = Vector3.zero
                Camera.CFrame = CFrame.new(Camera.CFrame.Position, Target.Character.Head.Position)
            end
            
            -- Aimlock Head (Otomatis Lock)
            if _G.Aimlock then
    ...
    Frame.Size = UDim2.new(0, 550, 0, 480); Frame.Position = UDim2.new(0.5, -275, 0.5, -240); Frame.BackgroundColor3 = Color3.fromRGB(5, 5, 5); Frame.ClipsDescendants = true
    Instance.new("UIStroke", Frame).Color = Color3.fromRGB(0, 255, 0)
    Frame.Active = true; Frame.Draggable = true

    -- ANIMASI PARTIKEL MELENGKET (CONNECTED)
    local Particles = {}
    for i = 1, 15 do
        local p = Instance.new("Frame", Frame); p.Size = UDim2.new(0, 4, 0, 4); p.BackgroundColor3 = Color3.fromRGB(0, 255, 0); p.Position = UDim2.new(math.random(), 0, math.random(), 0)
        Instance.new("UICorner", p).CornerRadius = UDim.new(1,0)
        Particles[i] = {Obj = p, Vel = Vector2.new(math.random(-1,1)/2, math.random(-1,1)/2)}
    end

    local function CreateTab(name, sidebar, func, index)
        local b = Instance.new("TextButton", sidebar); b.Size = UDim2.new(1, 0, 0, 40); b.Position = UDim2.new(0, 0, 0, index * 40)
        b.Text = name; b.TextColor3 = Color3.fromRGB(0, 255, 0); b.BackgroundColor3 = Color3.fromRGB(15, 15, 15); b.Font = Enum.Font.GothamBold; b.ZIndex = 5
        b.MouseButton1Click:Connect(func)
    end

    local Sidebar = Instance.new("Frame", Frame); Sidebar.Size = UDim2.new(0, 130, 1, -40); Sidebar.Position = UDim2.new(0, 0, 0, 40); Sidebar.BackgroundColor3 = Color3.fromRGB(10, 10, 10); Sidebar.ZIndex = 4
    local Content = Instance.new("ScrollingFrame", Frame); Content.Size = UDim2.new(1, -140, 1, -50); Content.Position = UDim2.new(0, 135, 0, 45); Content.BackgroundTransparency = 1; Content.ZIndex = 4; Content.CanvasSize = UDim2.new(0,0,2,0)
    Instance.new("UIListLayout", Content).Padding = UDim.new(0, 5)

    local function AddToggle(name, varName)
        local b = Instance.new("TextButton", Content); b.Size = UDim2.new(1, -10, 0, 35); b.Font = Enum.Font.GothamBold; b.ZIndex = 5
        local function Upd()
    ...
    -- FUNGSI TAB (ANTI-HAPUS)
    local function ShowAim() Content:ClearAllChildren(); Instance.new("UIListLayout", Content).Padding = UDim.new(0, 5); AddToggle("FlyKill Head", "FlyKill"); AddToggle("Aimlock Head", "Aimlock"); AddToggle("AimKeramat", "AimKeramat"); AddToggle("MS20 Pull (20m)", "MS20") end
    ...
    local function ShowCfg() Content:ClearAllChildren(); Instance.new("UIListLayout", Content).Padding = UDim.new(0, 5); AddToggle("Speed Run", "SpeedRunning"); AddToggle("Jump Multi", "JumpMulti"); AddToggle("Wallhack", "Wallhack"); AddToggle("Auto Wins", "AutoWins") end
    local function ShowSB() Content:ClearAllChildren(); Instance.new("UIListLayout", Content).Padding = UDim.new(0, 5); AddToggle("Auto Farm S.Bronx", "AutoFarmSB") end
    ...
    CreateTab("AIM", Sidebar, ShowAim, 0); CreateTab("VISUAL", Sidebar, ShowVis, 1); CreateTab("CONFIG", Sidebar, ShowCfg, 2); CreateTab("S.BRONX", Sidebar, ShowSB, 3); CreateTab("RINTANGAN", Sidebar, ShowRin, 4)
    ShowAim()

    local Title = Instance.new("TextLabel", Frame); Title.Size = UDim2.new(1, 0, 0, 40); Title.BackgroundColor3 = Color3.fromRGB(0, 40, 0); Title.Text = "VERCO X NUGI | ULTIMATE"; Title.TextColor3 = Color3.fromRGB(0, 255, 0); Title.Font = Enum.Font.GothamBold; Title.TextSize = 20; Title.ZIndex = 5
