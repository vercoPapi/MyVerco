...
local function StartHub()
    ...
    Container.Size = UDim2.new(1, -20, 1, -290)
    ...
    -- FITUR BARU: AIMBOT HEADLOCK
    local AimlockTarget = nil
    CreateCheckbox("AimBot (Head Lock)", function(v)
        _G.AimBot = v
    ...
    -- FITUR BARU: ESP LINE & BOX
    CreateCheckbox("ESP Line & Box", function(v)
        _G.ESP = v
        while _G.ESP do task.wait(0.5)
            for _, p in pairs(Players:GetPlayers()) do
                if p ~= LocalPlayer and p.Character and not p.Character:FindFirstChild("VercoESP") then
    ...
    -- FITUR BARU: ANTI JATUH MAP
    CreateCheckbox("Anti Jatuh Map", function(v)
        _G.AntiFall = v
        RunService.Stepped:Connect(function()
            if _G.AntiFall and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                if LocalPlayer.Character.HumanoidRootPart.Position.Y < -50 then
                    LocalPlayer.Character.HumanoidRootPart.Velocity = Vector3.new(0,0,0)
                    LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(LocalPlayer.Character.HumanoidRootPart.Position.X, 10, LocalPlayer.Character.HumanoidRootPart.Position.Z)
                end
            end
        end)
    end)

    -- FITUR TELEPORT (DIUBAH: TIDAK PINDAH SEBELUM MATI)
    local CurrentTarget = nil
    CreateCheckbox("Teleport (Wait Kill)", function(v)
        _G.TPBack = v
        task.spawn(function()
    ...
    -- FITUR AUTOMAKRO (DIUBAH: FAST KILL)
    CreateCheckbox("AutoMakro (Fast Kill)", function(v)
        _G.AutoMakro = v
        task.spawn(function()
            while _G.AutoMakro do task.wait()
                pcall(function()
                    ...
                    if tool then
                        for i=1, 5 do tool:Activate() end -- Rapid fire
    ...
    -- FITUR AIM MAGNET (DIUBAH: TARGET KE DEPAN)
    CreateCheckbox("AimMagnet (Pull Target)", function(v)
        _G.AimMagnet = v
        RunService.RenderStepped:Connect(function()
            if _G.AimMagnet then
                for _, p in pairs(Players:GetPlayers()) do
                    if p ~= LocalPlayer and p.Character and p.Character:FindFirstChild("HumanoidRootPart") then
                        ...
                        if d <= 100 then
                            p.Character.HumanoidRootPart.CFrame = LocalPlayer.Character.HumanoidRootPart.CFrame * CFrame.new(0, 0, -5)
                        end
                    end
                end
            end
    ...
    CreateCheckbox("Anti Crash & Lag", function(v) if v then setfpscap(120) settings().Physics.PhysicsEnvironmentalThrottle = Enum.EnviromentalPhysicsThrottle.Disabled end end)
    CreateCheckbox("No Reload", function(v) _G.NR = v while _G.NR do task.wait() pcall(function() LocalPlayer.Character:FindFirstChildOfClass("Tool").Ammo.Value = 999 end) end end)
