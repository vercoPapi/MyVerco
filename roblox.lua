...
-- UI ICON & MAIN (Tetap seperti aslinya)
...
-- ESP SUPER AGRESIF (Deteksi Segala Karakter/Bundle)
local function ApplyESP(plr)
    ...
    RS.RenderStepped:Connect(function()
        if plr ~= LP and plr.Character and (plr.Character:FindFirstChild("HumanoidRootPart") or plr.Character:FindFirstChildOfClass("BasePart")) then
            local Root = plr.Character:FindFirstChild("HumanoidRootPart") or plr.Character:FindFirstChildOfClass("BasePart")
...
-- LOGIC AIMBOT (LOCK & NO TEMBUS DINDING)
    ...
    for _, v in pairs(Players:GetPlayers()) do
        if v ~= LP and v.Character and v.Character:FindFirstChild("Head") and v.Character:FindFirstChildOfClass("Humanoid") and v.Character:FindFirstChildOfClass("Humanoid").Health > 0 then
            ...
            if vis then
                ...
                local res = workspace:Raycast(Camera.CFrame.Position, (head.Position - Camera.CFrame.Position).Unit * 1000, params)
                
                if res and res.Instance:IsDescendantOf(v.Character) then
...
RS.RenderStepped:Connect(function()
    ...
    if LP.Character and LP.Character:FindFirstChild("HumanoidRootPart") then
        ...
        if t and t.Character and t.Character:FindFirstChild("Head") then
            ...
            local targetRoot = t.Character:FindFirstChild("HumanoidRootPart") or targetHead

            -- MS100: Musuh otomatis muncul di depan
            ...
            -- Fly Kill: Di atas kepala (pas), lock head + auto shoot
            if States["Fly Kill"] then
                myRoot.CFrame = targetRoot.CFrame * CFrame.new(0, 7.5, 0)
                myRoot.Velocity = Vector3.new(0,0,0)
                Camera.CFrame = Camera.CFrame:Lerp(CFrame.new(Camera.CFrame.Position, targetHead.Position), 0.15)
                mouse1press() task.wait() mouse1release()
            end

            -- AimBot: Mengunci (Lock) & No tembus dinding
...
local Tabs = {
    ...
    ["Menu Config"] = function()
        ...
        CreateCheckbox("Anti lag", function(v) States["Anti lag"] = v end)
    end
}
