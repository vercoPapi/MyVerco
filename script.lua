...
-- Fungsi Wall Check (Raycast agar tidak lock tembus dinding)
        ...
        if v ~= Player and v.Character and v.Character:FindFirstChild("Humanoid") and v.Character.Humanoid.Health > 0 then
            ...
            if hrp then
                ...
                if Mag < shortestDist then
                    -- FlyKill mengabaikan wallcheck, Aimlock menggunakan wallcheck
                ...
                -- MS20 Logic (Teleport musuh 20m ke depan player)
...
local function ShowAim()
    ...
    Instance.new("UIListLayout", Content).Padding = UDim.new(0, 5)
    AddToggle("Aimlock (No FOV Check)", "Aimlock")
    AddToggle("FlyKill (Next-Target)", "FlyKill")
    AddToggle("MS20 (Pull Distance)", "MS20")
end

local function ShowVisual()
    Content:ClearAllChildren()
    Instance.new("UIListLayout", Content).Padding = UDim.new(0, 5)
    AddToggle("ESP 3D Wireframe Red", "EspEnabled")
end
