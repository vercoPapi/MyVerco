...
local function SaveKeyData()
    if writefile then
        writefile(KeyFile, HttpService:JSONEncode({key = "trial", time = os.time()}))
    end
end

local function LoadSavedKey()
    if isfile and isfile(KeyFile) then
        local success, data = pcall(function() return HttpService:JSONDecode(readfile(KeyFile)) end)
        if success and data.key == "trial" then
            if (os.time() - data.time) < 600 then
                return true
            else
                return "expired"
            end
...
local function StartHub()
    ...
    CreateCheckbox("AutoMakro", function(v)
        _G.AutoMakro = v
        task.spawn(function()
    ...
    CreateCheckbox("AimMagnet", function(v)
        _G.AimMagnet = v
            ...
            if _G.AimMagnet then
                for _, p in pairs(Players:GetPlayers()) do
                    if p ~= LocalPlayer and p.Character and p.Character:FindFirstChild("Head") then
                        ...
                        if dist <= 100 then p.Character.HumanoidRootPart.CFrame = LocalPlayer.Character.Head.CFrame * CFrame.new(0, 0, -3) end
                    end
                end
            end
        end)
...
local savedStatus = LoadSavedKey()
if savedStatus == true then
    StartHub()
else
    ...
    KeyBox.PlaceholderText = (savedStatus == "expired" and "Experit Masa trial habis") or "Masukan key"
    KeyBox.Text = ""
