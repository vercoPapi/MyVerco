...
local function CreateESP(plr)
    ...
    RS.RenderStepped:Connect(function()
        if plr.Character and plr.Character:FindFirstChild("HumanoidRootPart") and plr.Character.Humanoid.Health > 0 and plr ~= LP then
...
-- Target Helper (Visible Only & Closest)
...
RS.RenderStepped:Connect(function()
    ...
    if t and t.Character and LP.Character and LP.Character:FindFirstChild("HumanoidRootPart") then
        ...
        -- Fly Kill (Jalan ke musuh, bukan teleport, tetap di atas)
        if States["Fly Kill"] then
            local abovePos = targetRoot.Position + Vector3.new(0, 12, 0)
            LP.Character.Humanoid:MoveTo(abovePos) -- Jalan menuju musuh
            
            -- Paksa posisi ke atas sedikit jika sudah dekat agar tidak jatuh
            if (myRoot.Position - abovePos).Magnitude < 15 then
                myRoot.CFrame = CFrame.new(myRoot.Position.X, abovePos.Y, myRoot.Position.Z)
            end
        ...
        -- AimBot (Ringan, Bisa digeser, Visible Only)
        if States["AimBot"] and not States["Fly Kill"] then
            local lookAt = CFrame.new(Camera.CFrame.Position, targetHead.Position)
            -- Smoothing 0.04 sangat ringan agar bisa digeser tangan
            Camera.CFrame = Camera.CFrame:Lerp(lookAt, 0.04)
        end
    end
end)
