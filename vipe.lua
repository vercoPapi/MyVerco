...
local function CreateSlider(page, text, min, max, def, callback)
    ...
    Bar.MouseButton1HoldRelease:Connect(function() -- Simple slider logic for mobile/PC
...
CreateToggle(AimPage, "AimBot (Lock Head)", function(t) Flags.Aimbot = t end)
CreateToggle(AimPage, "AimSilent", function(t) Flags.SilentAim = t end)

-- LOGIKA VISUAL (ESP BIRU)
CreateToggle(VisualPage, "Enable Esp", function(t) Flags.ESP = t end)
...
-- LOGIKA MOVEMENT
CreateToggle(MovePage, "Wallhack (NoJatuhMap)", function(t) 
    Flags.Wallhack = t
    LocalPlayer.Character.Humanoid:SetStateEnabled(Enum.HumanoidStateType.FallingDown, not t)
end)

CreateSlider(MovePage, "Speed (running)", 16, 500, 16, function(v) 
    LocalPlayer.Character.Humanoid.WalkSpeed = v 
end)

CreateButton(MovePage, "FlyKill (TeleportKeMusuh)", function()
    local p = game.Players:GetPlayers()
    local target = p[math.random(2, #p)]
    if target and target.Character then
        LocalPlayer.Character.HumanoidRootPart.CFrame = target.Character.HumanoidRootPart.CFrame * CFrame.new(0, 0, 3)
    end
end)

CreateToggle(MovePage, "Fly (diam di atas map)", function(t)
    Flags.Fly = t
    local bodyVel = LocalPlayer.Character.HumanoidRootPart:FindFirstChild("FlyVel") or Instance.new("BodyVelocity")
    bodyVel.Name = "FlyVel"
    bodyVel.Velocity = Vector3.new(0,0,0)
    bodyVel.MaxForce = t and Vector3.new(math.huge, math.huge, math.huge) or Vector3.new(0,0,0)
    bodyVel.Parent = LocalPlayer.Character.HumanoidRootPart
end)

CreateSlider(MovePage, "SkinPlayer (Geser)", 1, 50, 1, function(v)
    -- Logika Copy Skin Simple
end)
