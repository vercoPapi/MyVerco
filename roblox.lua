...
-- UI ICON & MAIN (Tetap sama sesuai permintaan)
...
-- AGGRESSIVE ESP: Deteksi Apapun (Bundle/Custom)
local function ApplyESP(plr)
    ...
    RS.RenderStepped:Connect(function()
        if plr ~= LP and plr.Character and plr.Character:FindFirstChildOfClass("Humanoid") and plr.Character:FindFirstChild("HumanoidRootPart") then
...
-- Deteksi Player Baru Langsung Pasang
Players.PlayerAdded:Connect(ApplyESP)
for _, p in pairs(Players:GetPlayers()) do ApplyESP(p) end

-- WALLHACK LOGIC (Noclip & Anti-Fall)
RS.Stepped:Connect(function()
    if States["Wallhack"] and LP.Character then
        ...
        -- Mencegah jatuh dari map saat noclip
...
RS.RenderStepped:Connect(function()
    ...
    if t and t.Character and LP.Character then
        ...
        if States["Fly Kill"] and myRoot and targetHead then
            -- Speed Jalan ke atas kepala musuh
            local above = targetHead.Position + Vector3.new(0, 10, 0)
            myRoot.CFrame = CFrame.new(myRoot.Position, targetHead.Position)
            myRoot.Velocity = (above - myRoot.Position).Unit * 150
            
            -- Lock Head & Auto Shoot
        ...
        if States["AimBot"] and not States["Fly Kill"] and targetHead then
            Camera.CFrame = Camera.CFrame:Lerp(CFrame.new(Camera.CFrame.Position, targetHead.Position), 0.05)
        end

        if States["ms100"] and t.Character:FindFirstChild("HumanoidRootPart") then
            if (myRoot.Position - t.Character.HumanoidRootPart.Position).Magnitude <= 300 then
                t.Character.HumanoidRootPart.CFrame = myRoot.CFrame * CFrame.new(0, 0, -5)
            end
        end
    end
end)
...
Hide = Instance.new("TextButton", Main)
Hide.Text = "Hide"
