...
local function StartMainScript()
    ...
    -- LOGIC LOOP (AIM, FLYKILL, MS20, SPEED)
    RunService.RenderStepped:Connect(function()
        local Target = nil
        ...
        -- Speed Features
        if _G.SpeedRunning and Player.Character and Player.Character:FindFirstChild("Humanoid") then Player.Character.Humanoid.WalkSpeed = 80 end
        ...
        if _G.AutoWins then
            for _, obj in pairs(workspace:GetDescendants()) do
                if obj.Name:lower():find("finish") or obj.Name:lower():find("win") then Player.Character.HumanoidRootPart.CFrame = obj.CFrame break end
            end
        end
    end)
    ...
    local function AddToggle(name, varName)
        ...
        Update()
        b.MouseButton1Click:Connect(function() _G[varName] = not _G[varName] Update() end)
    end
