...
local function GetVisibleTarget()
    ...
    for _, v in pairs(Players:GetPlayers()) do
        if v ~= LP and v.Character and v.Character:FindFirstChild("Head") and v.Character.Humanoid.Health > 0 then
            ...
            if vis then
                -- Check Wall (Visibility check)
                local ray = Ray.new(Camera.CFrame.Position, (head.Position - Camera.CFrame.Position).Unit * 1000)
                local hit, hitPos = workspace:FindPartOnRayWithIgnoreList(ray, {LP.Character, v.Character})
...
RS.RenderStepped:Connect(function()
    ...
    if t and t.Character and LP.Character and LP.Character:FindFirstChild("HumanoidRootPart") then
        ...
        -- Fly Kill (Walk to target, then stay above, lock head)
        if States["Fly Kill"] then
            ...
            myRoot.Velocity = moveDir * 100 -- Simulate walking/moving fast
        ...
        -- AimBot (Smooth, Movable, Visible Only)
        if States["AimBot"] and not States["Fly Kill"] then
            if targetHead then
                local lookAt = CFrame.new(Camera.CFrame.Position, targetHead.Position)
                -- Smoothing 0.05 makes it light and movable
                Camera.CFrame = Camera.CFrame:Lerp(lookAt, 0.05)
            end
        end
    end
end)
