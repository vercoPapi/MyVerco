...
-- ================= MAIN SCRIPT (FULL LENGKAP TANPA HAPUS) =================
local function StartMainScript()
    ...
    -- Fungsi Wall Check (Agar Aimlock Gak Tembus Tembok)
    ...
    -- ================= ESP 3D WIREFRAME (FULL BOX + TRACER) =================
    local function CreateESP(v)
        local Lines = {}
        for i = 1, 12 do
            Lines[i] = Drawing.new("Line")
            Lines[i].Color = Color3.fromRGB(0, 255, 0)
        ...
        RunService.RenderStepped:Connect(function()
            if _G.EspEnabled and IsEnemy(v) and v.Character and v.Character:FindFirstChild("HumanoidRootPart") and v.Character:FindFirstChild("Humanoid") and v.Character.Humanoid.Health > 0 then
    ...
    -- ================= LOGIC LOOP (AIM, FLYKILL, MS20, CONFIG) =================
    RunService.RenderStepped:Connect(function()
        local Target = nil
        ...
        for _, v in pairs(game.Players:GetPlayers()) do
            if IsEnemy(v) and v.Character and v.Character:FindFirstChild("HumanoidRootPart") and v.Character.Humanoid.Health > 0 then
                ...
                if Mag < shortestDist then
                    if _G.FlyKill or _G.FlyPlayer or IsVisible(v.Character:FindFirstChild("Head")) then
                        shortestDist = Mag
                        Target = v
                    end
                end
    ...
    for i = 1, 15 do
        ...
        p.Size = UDim2.new(0, 2, 0, 2); p.BackgroundColor3 = Color3.fromRGB(0, 255, 0)
        local function anim()
    ...
    local function AddToggle(name, varName)
        ...
        local function Upd()
            ...
            b.BackgroundColor3 = _G[varName] and Color3.fromRGB(0, 100, 0) or Color3.fromRGB(20, 20, 20)
            if varName == "SpeedRunning" and not _G[varName] then Player.Character.Humanoid.WalkSpeed = 16 end
            if varName == "JumpMulti" and not _G[varName] then Player.Character.Humanoid.JumpPower = 50 end
        end
        Upd()
    ...
    local function ShowVisual() Content:ClearAllChildren(); Instance.new("UIListLayout", Content).Padding = UDim.new(0, 5)
        AddToggle("ESP 3D Wireframe", "EspEnabled") end

    local function ShowConfig() Content:ClearAllChildren(); Instance.new("UIListLayout", Content).Padding = UDim.new(0, 5)
        AddToggle("Speed Running (5x)", "SpeedRunning"); AddToggle("Speed Vehicle", "SpeedCar"); AddToggle("Jump Multi", "JumpMulti"); AddToggle("Wallhack", "Wallhack"); AddToggle("Auto Wins Global", "AutoWins") end

    local function ShowSouthBronx() Content:ClearAllChildren(); Instance.new("UIListLayout", Content).Padding = UDim.new(0, 5)
        AddToggle("Auto Farm SouthBronx", "AutoFarmSB") end
    ...
    local function CreateTab(name, func)
        ...
        b.Size = UDim2.new(1, 0, 0, 40); b.Text = name; b.TextColor3 = Color3.fromRGB(0, 255, 0); b.BackgroundColor3 = Color3.fromRGB(15, 15, 15); b.Font = Enum.Font.GothamBold
...
LoginBtn.MouseButton1Click:Connect(function()
    if KeyInput.Text == CurrentKey then SaveTime(); StartMainScript() else KeyInput.Text = "SALAH!" end
end)
