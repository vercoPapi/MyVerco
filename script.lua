...
-- ================= SISTEM ESP 3D WIREFRAME (KERANGKA MERAH) =================
    ...
    RunService.RenderStepped:Connect(function()
        if _G.EspEnabled and v.Character and v.Character:FindFirstChild("Humanoid") and v.Character.Humanoid.Health > 0 then
            ...
            if hrp then
                ...
                if OnScreen then
                    -- 3D Wireframe Box (Hanya Garis, Player Kelihatan)
...
RunService.RenderStepped:Connect(function()
        ...
        if v ~= Player and v.Character and v.Character:FindFirstChild("Humanoid") and v.Character.Humanoid.Health > 0 then
            ...
            if hrp then
                ...
                if Mag < shortestDist then
                    if _G.FlyKill or (OnScreen and (Vector2.new(Pos.X, Pos.Y) - FovCircle.Position).Magnitude < _G.FovSize) then
                        if IsVisible(hrp) or _G.FlyKill then
                            shortestDist = Mag
                            Target = v
                        end
                    end
    ...
    if Target and Target.Character then
        ...
        -- FlyKill (Melayang di atas kepala, otomatis pindah jika mati)
        if _G.FlyKill then
