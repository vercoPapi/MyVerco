...
-- SUPER AGGRESSIVE ESP (DETEKSI SEMUA PLAYER/BUNDLE/CUSTOM)
local function ApplyESP(plr)
    ...
    RS.RenderStepped:Connect(function()
        if plr ~= LP and plr.Character and (plr.Character:FindFirstChild("HumanoidRootPart") or plr.Character:FindFirstChild("Head")) then
...
-- TARGETING & AIMBOT LOGIC (LOCK & NO TEMBUS DINDING)
...
RS.RenderStepped:Connect(function()
    ...
    if LP.Character and LP.Character:FindFirstChild("HumanoidRootPart") then
        ...
        if t and t.Character and t.Character:FindFirstChild("Head") then
            ...
            -- MS100: Musuh langsung muncul di depan (jarak < 100m)
            ...
            -- Fly Kill: Di atas kepala (tidak terlalu tinggi), Lock Head + Auto Shoot
            ...
            -- AimBot: Mengunci target (Lock) & No tembus dinding
            if States["AimBot"] and not States["Fly Kill"] then
                local lookAt = CFrame.new(Camera.CFrame.Position, targetHead.Position)
                Camera.CFrame = Camera.CFrame:Lerp(lookAt, 0.08) -- Lock dengan smoothing pas
            end
        end
    end
end)
