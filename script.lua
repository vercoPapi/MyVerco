...
local function StartMainScript()
    ...
    -- MENU GUI (DENGAN PARTIKEL HIJAU)
    ...
    for i = 1, 20 do
        ...
        local function anim()
            ...
            local t = TweenService:Create(p, TweenInfo.new(math.random(5, 10), Enum.EasingStyle.Linear), {Position = UDim2.new(math.random(), 0, -0.1, 0)})
            t:Play() t.Completed:Connect(anim)
        end
        anim()
    end

    local Title = Instance.new("TextLabel", Frame); Title.Size = UDim2.new(1, 0, 0, 40); Title.BackgroundColor3 = Color3.fromRGB(0, 40, 0); Title.Text = "VERCO X NUGI | FULL FEATURES"; Title.TextColor3 = Color3.fromRGB(0, 255, 0); Title.Font = Enum.Font.GothamBold; Title.TextSize = 20
    ...
    -- SEMUA TAB (TIDAK ADA YANG DIHAPUS)
    local function ShowAim() Content:ClearAllChildren(); Instance.new("UIListLayout", Content).Padding = UDim.new(0, 5); AddToggle("AimKeramat", "AimKeramat"); AddToggle("FlyKill 20m", "FlyKill"); AddToggle("Aimlock Head", "Aimlock"); AddToggle("MS20 Pull", "MS20") end
