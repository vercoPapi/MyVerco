...
-- TARGETING & AIMBOT LOGIC (NO TEMBUS DINDING)
...
RS.RenderStepped:Connect(function()
    ...
    if LP.Character and LP.Character:FindFirstChild("HumanoidRootPart") then
        ...
        if t and t.Character and t.Character:FindFirstChild("Head") then
            ...
            -- MS100: Musuh langsung di depan (jarak < 100m)
            if States["ms100"] then
                local d = (myRoot.Position - targetRoot.Position).Magnitude
                if d <= 300 then -- 300 studs ≈ 100 meter
                    targetRoot.CFrame = myRoot.CFrame * CFrame.new(0, 0, -5)
                end
            end

            -- Fly Kill: Di atas kepala, tidak terlalu tinggi, lock head + shoot
